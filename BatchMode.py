...
import easyocr # <<< Thêm import EasyOCR
import threading # <<< Thêm import threading để tải model không block UI
...
class BatchModeAppRedesigned(QMainWindow):
    def __init__(self):
        ...
        self.yolo_model = None # Model phát hiện xe
        self.lp_model = None   # <<< Model phát hiện BIỂN SỐ
        self.ocr_reader = None # <<< OCR Reader
        self.models_loaded = False # <<< Cờ trạng thái tải model
        self.ocr_allowed_chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789' # Ký tự cho phép OCR biển số VN

        # --- Đường dẫn tới model phát hiện biển số ---
        # <<< !!! THAY ĐỔI ĐƯỜNG DẪN NÀY NẾU CẦN !!! >>>
        self.lp_model_path = "license_plate_detector.pt"
        # -------------------------------------------

        self.vehicle_classes_vn = {
            'car': 'ô tô', 'motorcycle': 'xe máy', 'bus': 'xe buýt', 'truck': 'xe tải',
        }
        self.target_vehicle_classes = ['car', 'motorcycle'] # Chỉ xử lý ô tô và xe máy để tìm biển số
        self.initUI()
        # Sử dụng threading để tải model không làm đơ UI
    ...
    def _load_models_background(self):
        """Tải các model trong một luồng riêng biệt."""
        try:
            ...
            if not os.path.exists(self.lp_model_path):
                 ...
                 # Cập nhật UI để thông báo lỗi (cần dùng signal/slot nếu muốn an toàn hơn)
                 self.status_label.setText(f"Lỗi: Không tìm thấy {os.path.basename(self.lp_model_path)}")
                 self.models_loaded = False
                 self.update_button_states() # Cập nhật lại trạng thái nút
                 # Không cần hiển thị QMessageBox ở đây vì đang ở luồng khác
                 return # Dừng nếu không có model biển số
            ...
            # Khởi tạo EasyOCR (có thể chọn gpu=True nếu có GPU và CUDA)
            self.ocr_reader = easyocr.Reader(['en'], gpu=False) # Dùng 'en' vì biển số VN dùng chữ Latinh
            print("[INFO] OCR Reader đã sẵn sàng.")

            self.models_loaded = True
            print("[INFO] Tất cả model đã sẵn sàng.")
            # Cập nhật UI (an toàn hơn là dùng signal/slot)
            ...
            self.update_button_states() # Cập nhật lại trạng thái nút
            # Không nên hiển thị QMessageBox ở đây


    def initUI(self):
        # ... (Phần lớn UI giữ nguyên như trước) ...
        ...
        # --- Nút Xử lý Nhận Dạng ---
        self.process_detect_button = QPushButton("Nhận dạng Phương tiện & Biển số") # Đổi tên nút
        self.process_detect_button.setFont(QFont('Segoe UI', 11, QFont.Bold))
        self.process_detect_button.setMinimumSize(250, 40) # Tăng chiều rộng
        ...
        """)
        self.process_detect_button.clicked.connect(self.process_vehicle_and_lp_detection) # Kết nối hàm mới
        self.process_detect_button.setEnabled(False) # Mặc định vô hiệu hóa
        process_buttons_layout.addWidget(self.process_detect_button)
        # --- Kết thúc Nút Xử lý Nhận Dạng ---

        # ... (Các nút Resize, Format giữ nguyên) ...
        ...
        # --- Status Bar ---
        self.status_label = QLabel("Đang tải models...") # Thông báo ban đầu
    ...
    # --- Các hàm add_files, select_output_directory, display_selected_image, ... giữ nguyên ---
    # --- Chỉ cần sửa update_button_states và set_controls_enabled ---
    ...
    def display_selected_image(self, current_item, previous_item):
        # (Giữ nguyên code - sử dụng cv_image_to_qpixmap và _original_pixmap)
        if current_item:
            ...
            if full_path and os.path.exists(full_path):
                try:
                    ...
                    if not pixmap.isNull():
                        ...
                        # Chỉ cập nhật status nếu model đã sẵn sàng
                        if self.models_loaded:
                            self.status_label.setText(f"Xem trước: {file_name}")
                        else:
                            self.status_label.setText(f"Xem trước: {file_name} (Models đang tải...)")
                        ...
                        if hasattr(self.current_image_label, '_original_pixmap'):
                           del self.current_image_label._original_pixmap
             ...
             if hasattr(self.current_image_label, '_original_pixmap'):
                del self.current_image_label._original_pixmap
             # Cập nhật status nếu model đã sẵn sàng
             if self.models_loaded:
                 self.status_label.setText("Sẵn sàng.")
             else:
                 self.status_label.setText("Models đang tải...")
    ...
    def _prepare_processing(self, operation_name):
        ...
        if not self.output_directory:
            ...
            return False
        # Kiểm tra xem các model đã tải xong chưa nếu là xử lý nhận dạng
        if operation_name == "nhận dạng phương tiện và biển số" and not self.models_loaded:
             # Kiểm tra lại lần nữa phòng trường hợp luồng tải chưa xong hẳn
             if not self.yolo_model or not self.lp_model or not self.ocr_reader:
                 QMessageBox.warning(self, "Models chưa sẵn sàng", "Các model cần thiết (phương tiện, biển số, OCR) đang được tải hoặc đã xảy ra lỗi. Vui lòng chờ hoặc kiểm tra console.")
                 return False
             else: # Nếu biến models_loaded chưa kịp cập nhật nhưng model đã có thì vẫn cho chạy
                 self.models_loaded = True
    ...
    def update_button_states(self):
         ...
         # Nút xử lý nhận dạng chỉ bật khi có file, thư mục VÀ models đã tải xong
    ...
    def set_controls_enabled(self, enabled):
        ...
        self.resize_button.setEnabled(can_process_basic)
        self.format_button.setEnabled(can_process_basic)
        # Nút xử lý nhận dạng chỉ bật khi điều kiện cơ bản OK VÀ models đã tải xong
        self.process_detect_button.setEnabled(can_process_basic and self.models_loaded)

    # --- Các hàm process_resize, process_format_change giữ nguyên ---
    def process_resize(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            try:
                ...
                self.add_result_item(None, base_name, f"LỖI resize: {e}", file_path)
    ...
    # --- Hàm xử lý nhận diện XE và BIỂN SỐ ---
    def process_vehicle_and_lp_detection(self):
        """Thực hiện nhận diện phương tiện VÀ biển số trong ảnh."""
        operation_name = "nhận dạng phương tiện và biển số"
        if not self._prepare_processing(operation_name):
            return
        ...
        for i, file_path in enumerate(self.files):
            ...
            output_filename = f"{name_part}_detected_recognized{ext_part}"
            output_path = os.path.join(self.output_directory, output_filename)
            ...
            try:
                ...
                detection_summary = [] # List để lưu tóm tắt kết quả cho ảnh này

                img_draw = img_cv.copy() # Tạo bản sao để vẽ lên

                for result in vehicle_results:
                    boxes = result.boxes
                    for box in boxes:
                        x1_v, y1_v, x2_v, y2_v = map(int, box.xyxy[0]) # Tọa độ xe
                        conf_v = float(box.conf[0])
                        cls_v = int(box.cls[0])
                        class_name_v = self.yolo_model.names[cls_v]

                        # Chỉ xử lý các lớp xe mục tiêu và có độ tin cậy đủ cao
                        if class_name_v in self.target_vehicle_classes and conf_v > 0.5:
                            ...
                            vehicle_label = f"{class_name_vn}: {conf_v:.2f}"
                            vehicle_color = (0, 255, 0) # Màu xanh lá cho xe

                            # Vẽ hộp bao xe
                            cv2.rectangle(img_draw, (x1_v, y1_v), (x2_v, y2_v), vehicle_color, 2)
                            # Vẽ nhãn xe (đặt phía trên hộp)
                            ...
                            # --- 2. Nhận diện biển số BÊN TRONG vùng xe ---
                            vehicle_roi = img_cv[y1_v:y2_v, x1_v:x2_v] # Cắt vùng ảnh xe
                            if vehicle_roi.size == 0: continue # Bỏ qua nếu vùng cắt rỗng

                            lp_results = self.lp_model(vehicle_roi, verbose=False, conf=0.4) # Ngưỡng conf cho biển số có thể thấp hơn
                            best_lp_box = None
                            max_conf_lp = 0

                            # Tìm biển số có độ tin cậy cao nhất trong ROI xe
                            for lp_result in lp_results:
                                for lp_box in lp_result.boxes:
                                     if lp_box.conf[0] > max_conf_lp:
                                         max_conf_lp = lp_box.conf[0]
                                         best_lp_box = lp_box

                            recognized_plate = "N/A" # Giá trị mặc định nếu không nhận dạng được
                            if best_lp_box is not None:
                                # Tọa độ biển số (local trong ROI xe)
                                lp_x1, lp_y1, lp_x2, lp_y2 = map(int, best_lp_box.xyxy[0])

                                # --- 3. Cắt và xử lý ảnh biển số cho OCR ---
                                ...
                                if lp_image.size > 0:
                                    ...
                                    # Có thể thêm các bước tiền xử lý khác ở đây nếu cần:
                                    # ví dụ: làm nét, tăng tương phản, thresholding...
                                    ...
                                    try:
                                        ocr_result = self.ocr_reader.readtext(gray_lp,
                                                                              detail=0,
                                                                              paragraph=False,
                                                                              allowlist=self.ocr_allowed_chars)

                                        if ocr_result:
                                            # Nối các phần nhận dạng được và chuẩn hóa
                                            raw_text = "".join(ocr_result).upper().replace(" ", "")
                                            # Áp dụng định dạng cơ bản (ví dụ: thêm gạch nối) - có thể cần phức tạp hơn
                                            # if len(raw_text) > 5 and raw_text[2].isdigit() and raw_text[3].isalpha(): # Ví dụ 59A12345
                                            #      recognized_plate = f"{raw_text[:2]}-{raw_text[2:4]} {raw_text[4:]}" # Cần chuẩn hóa tốt hơn
                                            # else:
                                            recognized_plate = raw_text # Giữ nguyên nếu không khớp mẫu

                                            # Thêm vào tóm tắt kết quả
                                            detection_summary.append(f"{class_name_vn}: {recognized_plate}")

                                            # --- 5. Vẽ kết quả lên ảnh ---
                                            # Tính tọa độ biển số (global trên ảnh gốc)
                                            g_lp_x1 = x1_v + lp_x1
                                            g_lp_y1 = y1_v + lp_y1
                                            g_lp_x2 = x1_v + lp_x2
                                            g_lp_y2 = y1_v + lp_y2
                                            lp_color = (255, 0, 0) # Màu xanh dương cho biển số

                                            # Vẽ hộp bao biển số
                                            cv2.rectangle(img_draw, (g_lp_x1, g_lp_y1), (g_lp_x2, g_lp_y2), lp_color, 2)

                                            # Vẽ số xe nhận dạng được (đặt phía dưới biển số)
                                        ...
                                        else: # Nếu OCR không đọc được gì
                                             detection_summary.append(f"{class_name_vn}: (LP detected, OCR failed)")
                                    except Exception as ocr_err:
                                        print(f"Lỗi OCR cho {base_name}: {ocr_err}")
                                        detection_summary.append(f"{class_name_vn}: (LP detected, OCR error)")
                                else: # Nếu ảnh biển số cắt ra bị rỗng
                                     detection_summary.append(f"{class_name_vn}: (LP region empty)")

                            else: # Nếu không phát hiện biển số trong xe
                                detection_summary.append(f"{class_name_vn}: (LP not found)")

                # --- Lưu ảnh đã vẽ kết quả ---
                ...
                if not result_pixmap.isNull():
                    ...
                    self.add_result_item(None, base_name, "LỖI: Không tạo được ảnh xem trước sau xử lý", file_path)

            except Exception as e:
                print(f"Lỗi nghiêm trọng khi xử lý file {base_name}: {e}")
                import traceback
                traceback.print_exc()
                self.add_result_item(None, base_name, f"LỖI xử lý: {e}", file_path)
    ...
    # --- Hàm add_result_item, clear_results, keyPressEvent, resizeEvent giữ nguyên ---
    def add_result_item(self, pixmap, original_name, info_text, result_path):
        ...
        img_label.setMinimumSize(180, 120)
    ...
    def closeEvent(self, event):
        """Đảm bảo luồng tải model dừng nếu đang chạy."""
        # Mặc dù là daemon thread, việc join() có thể hữu ích nếu cần chờ dọn dẹp
        # if self.load_models_thread and self.load_models_thread.is_alive():
        #     print("[INFO] Stopping model loading thread...")
        #     # Không có cách trực tiếp để dừng thread, nhưng daemon sẽ tự thoát
        super().closeEvent(event)


if __name__ == '__main__':
    # (Phần kiểm tra thư viện giữ nguyên - thêm easyocr nếu cần)
    missing_libs = []
    ...
    if missing_libs:
        ...
        # ... (in các lệnh pip install tương ứng) ...
